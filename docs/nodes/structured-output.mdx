...
* **Temperature**:
    ...
    Adjusts the randomness of the model's output (0.0–2.0). **Recommendation:** use **0** (or very low, e.g. 0.1) for deterministic extraction so the same input yields consistent structured output; higher values are not recommended for this node.
