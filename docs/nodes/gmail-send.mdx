...
- **Recipients**:
  ...
  A comma-separated list of valid email addresses without spaces. Supports format strings from workflow input (e.g. `{email}`).
...
- **Body**:
  ...
  The main content of the email. Supports format strings from workflow input (e.g. `{llm_output}`).
